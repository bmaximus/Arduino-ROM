...
byte mac[] = { 
  0x00, 0xAA, 0xBB, 0x08, 0xDE, 0x02 };
byte service[] = { 
  83, 212, 84, 224};   //okeanos
...
void loop () {
  ...
  {                                                                                 // wste na stalthei to URL aitima sto Server
    ...
    if (client.connect(service, 8081)) {                                  //elegxos ean i sindesi itan success me ton server
      ...
      client.println("Host: http://83.212.84.224:8081");
      Serial.println("Host: http://83.212.84.224:8081");
